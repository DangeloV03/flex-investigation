...
#
# Equilibrium (exact mu = 2*eps, Ising limit) susceptibility run at 7.5x the
...
# 7.5x the original baseline, eq AND prod uniformly dilated (restores original
# 2:1 prod:eq ratio). PROD_CHUNKS = 3000 gives chunk_time = PROD/CHUNKS = 500
