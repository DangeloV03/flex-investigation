"""
plot_peak_chi_multiplier_comparison.py

Overlay peak-chi-vs-L curves from a set of run-time multiplier runs on a single
log-log axis, to see whether chi^max(L) grows with run length (under-converged /
not yet equilibrated) or collapses onto one curve (time-converged). Works for any
multiplier family (the original 2x-5x prod study, the driven 1x/2x/3x eq+prod
study, the equilibrium 7p5x/10x/12p5x study, ...) via --mults.
...
with columns L, epsilon, chi_mean, chi_stderr. Run plot_susceptibility.py --pooled
once per multiplier first.

Usage:
    python susceptibility/plot_peak_chi_multiplier_comparison.py \
        --plots-root plots/eq_timetest --mults 7p5x 10x 12p5x \
        --out plots/eq_timetest/peak_chi_vs_L_comparison.png
"""
