...
#
# Equilibrium (exact mu = 2*eps, Ising limit) susceptibility run at 10x the
...
# 10x the original baseline, eq AND prod uniformly dilated (restores original
# 2:1 prod:eq ratio). PROD_CHUNKS = 4000 gives chunk_time = PROD/CHUNKS = 500
