...
#
# Equilibrium (exact mu = 2*eps, Ising limit) susceptibility run at 12.5x the
# ORIGINAL baseline time (EQ 100000, PROD 200000), for the long-timescale
# equilibration study: does chi^max(L) keep growing or converge when both eq and
# prod time are dilated far past the multiplier_test 2x-5x range?
# Single epsilon, looping square sizes L = 16 .. 256. Submitted once per epsilon by
...
# 12.5x the original baseline, eq AND prod uniformly dilated (restores original
# 2:1 prod:eq ratio). PROD_CHUNKS = 5000 gives chunk_time = PROD/CHUNKS = 500
# (5x fewer samples than the chunk_time=100 convention -- enough for chi^max).
EQ_TIME=1250000.0
PROD_TIME=2500000.0
PROD_CHUNKS=5000
SEED_BASE=7000
