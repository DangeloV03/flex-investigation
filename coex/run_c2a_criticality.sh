#!/usr/bin/env bash
# Experiment C2A: bimodality criticality per slab size, then FSS across sizes.
#
# Physics: scheme=homo, βΔf=0, k=1, βΔμ=1
# Reads  COEX_RUNS_C2A/ly<N>/{results,manage.csv}  from ./coex/run_c2a_coex_campaign.sh
# Writes COEX_RUNS_C2A/criticality/ly<N>/{bc_vs_beta_epsilon.csv,criticality.csv,*.png}
#        COEX_RUNS_C2A/criticality/multi_L/   (βε_c and βμ_coex(ε_c) vs L)
#
# Usage (after every Ly shows mu_coex_fitted=81 in the campaign status):
#   ./coex/run_c2a_criticality.sh              # Ly 16 20 40
#   ./coex/run_c2a_criticality.sh run 20 40    # subset
#   ./coex/run_c2a_criticality.sh compare      # ε_c table + FSS plots
...
echo
echo "Done. Compare with:  ./coex/run_c2a_criticality.sh compare"
