#!/usr/bin/env bash
# Scheme-3 (negative_drive) coexistence at multiple slab sizes, fixed Δμ=1.0.
#
# Physics identity: scheme=negative_drive, Δf=0, Δμ=1.0, k=0.1, flex-scheme=3
# Grid: ε ∈ [-2.5, -1.6] step 0.005 (finer than the original scheme3 0.01 sweep),
# initial μ window ±0.05, 10 μ points. Lx = 10 * Ly.
#
...
#
# Layout (repo-root relative; separate from the flat scheme3/dmu1p0/ Ly=16 tree):
#   scheme3/dmu1p0_multiL/ly<N>/{samples,results,manage.csv,queue.json}
#
# Usage (on Della login node, from repo root after sourcing env.sh):
#   ./coex/run_scheme3_multi_L_campaign.sh generate           # seed all default L
#   ./coex/run_scheme3_multi_L_campaign.sh generate 20 40     # only new sizes
#   ./coex/run_scheme3_multi_L_campaign.sh daemons
#   ./coex/run_scheme3_multi_L_campaign.sh all 20 40
#   ./coex/run_scheme3_multi_L_campaign.sh status
#   tmux attach -t scheme3-dmu1-multiL
#   tmux kill-session -t scheme3-dmu1-multiL
#
# After coex finishes:
#   ./coex/run_scheme3_multi_L_criticality.sh
...
base_for() { echo "$COEX_ROOT/$(ly_tag "$1")"; }
...
generate() {
  ...
  for ly in "${LYS[@]}"; do
    ...
    echo "== generating $base (Ly=$ly, Lx=$((10 * ly)), Δμ=$DELTA_MU, mu_window=±$MU_WINDOW) =="
